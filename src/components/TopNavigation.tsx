...
import { Menu, Shield, Settings, Moon, Sun, TrendingUp, Wallet, Bell, Users, Languages, ChevronDown, Search } from 'lucide-react';
...
export default function TopNavigation({ isDarkMode, onToggleDarkMode, userBalance }: TopNavigationProps) {
  ...
  return (
    <>
            ...
            <div className="lg:hidden">
              <Sheet open={showMobileMenu} onOpenChange={setShowMobileMenu}>
                ...
                <SheetContent side="right" className="w-80">
                  ...
                  <NavAccordion handleLinkClick={(page) => {
                    handleNavClick(`/${page}`);
                  }} />
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-card/95 backdrop-blur supports-[backdrop-filter]:bg-card/60 border-t border-border/40">
        <div className="flex flex-row justify-between items-center p-2 w-full" style={{ height: '70px' }}>
          {mainNavItems.map((item) => {
            const Icon = item.icon;
            const active = isActive(item.path);
          ...
          <Button
            ...
            onClick={() => {
              ...
              if (location.pathname === '/') {
                ...
                setTimeout(() => {
                  const searchInput = document.getElementById('markets-search-input');
                  if (searchInput) {
                    searchInput.focus();
                  }
                }, 300);
          ...
          >
            ...
            <span className="text-xs font-medium truncate" style={{ fontSize: '12px' }}>Search</span>
          </Button>
        </div>
      </div>
    </>
